...
OUTPUT_DIR = '/Users/jaehoon/Alphatross/70_Research/checkgpt/rivision/Jaccard_Analysis_Claude'
INVESTOR_TYPES = [
    "neutral_investor", 
    "value_investor", 
    "growth_investor",
...
def analyze_portfolios(data, subset_size=None, output_suffix=""):
    ...
    for investor_type in INVESTOR_TYPES:
        ...
        title = f"{investor_type} - Claude Repetition Analysis (n={len(portfolios)})"
...
def create_comparison_plots(results_50, results_100):
    ...
    plt.title("Claude - Jaccard Similarity Mean: 50 reps vs 100 reps")
    ...
    plt.title("Claude - Jaccard Similarity SD: 50 reps vs 100 reps")
    ...
    plt.title("Claude - Number of Modes: 50 reps vs 100 reps")
...
def main():
    ...
    print(f"Loading Claude portfolio data from {RESULT_FILE}...")
    ...
    if os.path.exists(multimodal_summary_path):
        ...
        if multimodal_pct > 20:
            print("  FINDING: Significant multimodality detected in Claude distributions.")
            print("  INTERPRETATION: The variation in Claude portfolios shows multiple distinct patterns/clusters.")
        else:
            print("  FINDING: Limited multimodality in Claude distributions.")
            print("  INTERPRETATION: The variation in Claude portfolios appears to be random rather than clustered.")
