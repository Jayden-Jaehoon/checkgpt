...
def main():
    # Load the data file
    file_path = '/Users/jaehoon/Alphatross/70_Research/checkgpt/results/Rephrase/Rephrase_Repetition_Result_NVG_claude.json'
    ...
    output_dir = '/Users/jaehoon/Alphatross/70_Research/checkgpt/rivision/Rephrase_Variability/claude_jaccard'
        ...
        for portfolio_size in portfolio_sizes:
            ...
            for prompt_key, prompt_title in prompt_types.items():
                if prompt_key in data:
                    ...
                    txt_path = os.path.join(output_dir, f'claude_{sample_suffix}_{portfolio_size}_{prompt_key}_jaccard_matrix_formatted.txt')
                    with open(txt_path, 'w') as f:
                        f.write(table)
                    print(f"Formatted table saved to {txt_path}")

                    # Save the matrix as a CSV
                    csv_path = os.path.join(output_dir, f'claude_{sample_suffix}_{portfolio_size}_{prompt_key}_jaccard_matrix.csv')
            ...
            output_path = os.path.join(output_dir, f'claude_{sample_suffix}_{portfolio_size}_jaccard_matrices_comparison.png')
            ...
            stats_csv_path = os.path.join(output_dir, f'claude_{sample_suffix}_{portfolio_size}_jaccard_similarity_stats.csv')
            stats_df.to_csv(stats_csv_path, index=False)
            print(f"Statistics saved to {stats_csv_path}")

            # Plot statistics
            stats_plot_path = os.path.join(output_dir, f'claude_{sample_suffix}_{portfolio_size}_jaccard_similarity_stats_plot.png')
